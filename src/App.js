                ...
                <div key={i} style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 12, padding: "16px" }}>
                  <div style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: "rgba(255,255,255,.52)", lineHeight: 1.5 }}>{s.descLanding || s.desc}</div>
                    </div>
                  </div>
                  {/* Estudios: 3D en landing */}
                  {s.subsections && s.subsections[0]?.imgs3d && s.title === "Alquiler de Estudios" && s.subsections.map((sub, si) => (
                    <div key={si}>
                      <div className="sub-label">{sub.label}</div>
                      <Carousel3D imgs={sub.imgs3d} />
                    </div>
                  ))}
                  {/* Producción: 3D en landing */}
                  {s.subsections && s.subsections[0]?.imgs3d && s.title === "Producción Musical" && s.subsections.map((sub, si) => (
                    <div key={si}>
                      <div className="sub-label">{sub.label}</div>
                      <Carousel3D imgs={sub.imgs3d} />
                    </div>
