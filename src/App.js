...
function Ecosystem() {
  ...
  const nodes = [
    { label: "Estudio", icon: "🎙️", color: "#8a0c0c", desc: "Grabación profesional" },
    { label: "Legal", icon: "⚖️", color: "#6b0606", desc: "Protección musical" },
    { label: "Marketing", icon: "📲", color: "#7a0a0a", desc: "Estrategia digital" },
    { label: "Producción", icon: "🎵", color: "#590707", desc: "TunyD & Money Makers" },
    { label: "Mezcla", icon: "🎚️", color: "#6b0606", desc: "Master en 72h" },
    { label: "Distribución", icon: "🌎", color: "#7a0a0a", desc: "Presencia global" },
  ];
  ...
  return (
    <div style={{ position: "relative", width: "100%", paddingBottom: "90%", margin: "16px 0" }}>
      <svg viewBox="0 0 100 100" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        <defs>
          <radialGradient id="eglow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#590707" stopOpacity="0.25" />
        ...
        {/* Background glow */}
        <circle cx={cx} cy={cy} r="45" fill="url(#eglow)" />

        {/* Orbit rings */}
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="rgba(89,7,7,.2)" strokeWidth="0.3" strokeDasharray="2 1" />
        <circle cx={cx} cy={cy} r={r * 0.6} fill="none" stroke="rgba(89,7,7,.12)" strokeWidth="0.2" />

        {/* Connection lines to active node */}
        {nodes.map((n, i) => {
          const a = (i * 360 / nodes.length + angle) * Math.PI / 180;
          const nx = cx + r * Math.cos(a); const ny = cy + r * Math.sin(a);
          return (
            <line key={i} x1={cx} y1={cy} x2={nx} y2={ny}
              stroke={i === active % nodes.length ? "rgba(255,60,60,.6)" : "rgba(89,7,7,.2)"}
              strokeWidth={i === active % nodes.length ? 0.5 : 0.2}
        ...
        {nodes.map((n, i) => {
          const a = (i * 360 / nodes.length + angle) * Math.PI / 180;
          const nx = cx + r * Math.cos(a); const ny = cy + r * Math.sin(a);
          const isActive = i === active % nodes.length;
          return (
            <g key={i}>
              <circle cx={nx} cy={ny} r={isActive ? 6.5 : 5} fill={n.color}
                style={{ transition: "all .5s", filter: isActive ? "drop-shadow(0 0 3px rgba(255,80,80,.8))" : "none" }}
              />
              <text x={nx} y={ny} textAnchor="middle" dominantBaseline="middle"
                fontSize="4" style={{ userSelect: "none" }}>{n.icon}</text>
              {isActive && (
                <text x={nx} y={ny + 8} textAnchor="middle" fontSize="2.5"
                  fill="rgba(255,255,255,.9)" fontFamily="Raleway,sans-serif" fontWeight="700">{n.label}</text>
              )}
            </g>
          );
        })}
        ...
        {/* Center node — KAPITAL */}
        <circle cx={cx} cy={cy} r="10" fill="#590707" filter="url(#fglow)">
          <animate attributeName="r" values="10;11.5;10" dur="2s" repeatCount="indefinite"/>
        </circle>
        <circle cx={cx} cy={cy} r="13" fill="none" stroke="rgba(89,7,7,.5)" strokeWidth="0.4">
          <animate attributeName="r" values="11;14;11" dur="2s" repeatCount="indefinite"/>
          <animate attributeName="opacity" values="0.8;0;0.8" dur="2s" repeatCount="indefinite"/>
        </circle>
        <text x={cx} y={cy - 1} textAnchor="middle" dominantBaseline="middle"
          fontSize="3" fill="#fff" fontWeight="900" fontFamily="Raleway,sans-serif" letterSpacing="0.3">KAPITAL</text>
        <text x={cx} y={cy + 3.5} textAnchor="middle" dominantBaseline="middle"
          fontSize="2" fill="rgba(255,255,255,.6)" fontFamily="Raleway,sans-serif">MUSIC</text>
      </svg>

      {/* Active node description */}
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, textAlign: "center" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(89,7,7,.25)", border: "1px solid rgba(89,7,7,.4)", borderRadius: 20, padding: "5px 14px" }}>
          <span style={{ fontSize: 14 }}>{nodes[active % nodes.length].icon}</span>
          <span style={{ fontSize: 11, fontWeight: 700, color: "#fff" }}>{nodes[active % nodes.length].label}</span>
          <span style={{ fontSize: 10, color: "rgba(255,255,255,.5)" }}>— {nodes[active % nodes.length].desc}</span>
        </div>
      </div>
    </div>
  );
      ...
      {phase === "landing" && (
        <div style={{ width: "100%" }}>
          <div className="hg" style={{ position: "relative", padding: `48px ${P} 36px`, overflow: "hidden" }}>
            ...
            {/* LOGO MARCA DE AGUA */}
            <img src={LOGO_URL} alt="" style={{ position: "absolute", right: -20, top: "50%", transform: "translateY(-50%)", width: 220, height: 220, objectFit: "contain", opacity: 0.07, pointerEvents: "none" }} />
