...
const SERVICES = [
  {
    ...
    desc: "Estudio A (premium) y Estudio B disponibles. Solo incluye el espacio — trae tu ingeniero o contrata uno por aparte.",
    subsections: [
      {
        label: "Estudio A", info: ESTUDIO_A_INFO,
        imgs: [CL+"estudio-a-1_yitif2",CL+"estudio-a-2_ax1jy5",CL+"estudio-a-3_wnlvfd",CL+"estudio-a-4_ypaaea"]
      },
      {
        label: "Estudio B", info: ESTUDIO_B_INFO,
        imgs: [CL+"estudio-b-1_hlduee",CL+"estudio-b-2_wpzby5",CL+"estudio-b-3_gqeg4h",CL+"estudio-b-4_asp8p4"]
      }
    ]
  },
  {
    icon: "⚖️", title: "Asesoría Legal", tag: "PROTECCIÓN",
    desc: "Protección completa para tu música y tu carrera artística.",
    legalInfo: [
      { title: "Derechos Patrimoniales y Conexos", items: ["Registro obra musical en la DNDA", "Registro de fonograma"] },
      { title: "Afiliaciones", items: ["SAYCO", "ACINPRO", "ASCAP"] },
      { title: "Documentos Artísticos", items: ["Creación de documentos para desarrollo de actividades artísticas y relacionadas"] },
      { title: "Split Sheet & Contratos", items: ["Split Sheet", "Reléase", "Declaración de puntos de máster", "Acuerdos y contratos"] },
    ],
    imgs: []
  },
  { icon: "🎚️", title: "Mezcla & Mastering", tag: "PRODUCCIÓN", desc: "¿Ya tienes tus temas grabados? Nuestros ingenieros los llevan al nivel internacional que merecen. Entrega en 72h.", imgs: [] },
  { icon: "📲", title: "Marketing 30", tag: "CRECIMIENTO", desc: "Análisis de contenido, crecimiento orgánico, pauta en Meta, parrilla mensual y asesoramiento de marca. Un mes completo de activación.", imgs: [] },
  {
    ...
    subsections: [
      { label: "Money Makers", imgs3d: [CL+"money-makers-1_ezbtfk", CL+"money-makers-3_oba1ls", CL+"money-makers-2_e8zimp"] },
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .album-cover { width: 80px; height: 80px; border-radius: 8px; object-fit: cover; flex-shrink: 0; border: 1px solid rgba(255,255,255,.1); }
    ...
    <div style={{ minHeight: "100vh", background: "#080808", color: "#fff", fontFamily: "'Raleway',sans-serif", width: "100%", overflowX: "hidden" }}>
      ...
      <nav className="nav">
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <img src={LOGO_URL} alt="Kapital Music" style={{ width: 48, height: 48, objectFit: "contain" }} onError={e=>{e.target.style.display='none';}} />
          <div>
      ...
      {phase === "landing" && (
        <div style={{ width: "100%" }}>
          ...
          <div style={{ padding: `20px 0`, overflow: "hidden" }}>
            <div style={{ padding: `0 ${P}`, marginBottom: 10 }}>
              <div className="tag tag-g" style={{ marginBottom: 6 }}>+250 CANCIONES PRODUCIDAS</div>
              <p style={{ fontSize: 12, color: "rgba(255,255,255,.4)" }}>Parte de nuestro catálogo</p>
            </div>
          ...
          <div style={{ padding: `0 ${P} 32px` }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {SERVICES.map((s, i) => (
                <div key={i} style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 12, padding: "16px" }}>
                  ...
                  {s.subsections && !s.subsections[0]?.imgs3d && s.subsections.map((sub, si) => (
                    <div key={si}>
                      ...
                      {sub.info && (
                        <div style={{ background: "rgba(89,7,7,.1)", border: "1px solid rgba(89,7,7,.25)", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
                          {sub.info.map((eq, ei) => (
                            <div key={ei} className="equip-item"><span style={{ color: "#590707", fontWeight: 900 }}>*</span>{eq}</div>
                          ))}
                        </div>
                      )}
                  ...
                  {s.legalInfo && (
                    <div style={{ marginTop: 4 }}>
                      {s.legalInfo.map((section, si) => (
                        <div key={si} className="legal-section">
                          <div className="legal-section-title">{section.title}</div>
                          {section.items.map((item, ii) => (
                            <div key={ii} className="legal-item"><span style={{ color: "#590707", fontWeight: 900, flexShrink: 0 }}>·</span>{item}</div>
                          ))}
                        </div>
                      ))}
                    </div>
