...
function Ecosystem() {
  ...
  const satellites = [
    { label: "Studio", icon: "🎙️", desc: "Grabación profesional de alto nivel", angle: 0 },
    { label: "Legal", icon: "⚖️", desc: "Protección total de tu música", angle: 60 },
    { label: "Marketing", icon: "📲", desc: "Estrategia digital y visibilidad", angle: 120 },
    { label: "Digital", icon: "🌐", desc: "Distribución en todas las plataformas", angle: 180 },
    { label: "Management", icon: "🤝", desc: "Acompañamiento en tu carrera", angle: 240 },
    { label: "Sync", icon: "🎬", desc: "Oportunidades en cine y medios", angle: 300 },
  ];

  const bentoCards = [
    { icon: "🎙️", title: "Estudio Profesional", desc: "Dos estudios totalmente equipados para producciones de la más alta calidad. El ambiente que tu música merece.", color: "#C0392B" },
    { icon: "⚖️", title: "Blindaje Legal", desc: "Registra, protege y monetiza tu música correctamente. Porque el talento sin estructura legal pierde valor.", color: "#8a0c0c" },
    { icon: "📲", title: "Marketing 360°", desc: "Pauta inteligente, contenido estratégico y crecimiento orgánico real.", color: "#C0392B" },
    { icon: "🎵", title: "Producción Elite", desc: "TunyD y Money Makers. Del concepto al master en un solo lugar.", color: "#8a0c0c" },
  ];
  ...
  return (
    <div style={{ margin: "20px 0" }}>
      {/* ORBITAL SYSTEM */}
      <div style={{ position: "relative", width: "100%", paddingBottom: "100%", maxWidth: 340, margin: "0 auto 24px" }}>
        <div style={{ position: "absolute", inset: 0 }}>
          <svg viewBox="0 0 100 100" style={{ width: "100%", height: "100%", overflow: "visible" }}>
            ...
            {/* Orbit path */}
            <circle cx="50" cy="50" r="34" fill="none" stroke="rgba(192,57,43,.15)" strokeWidth="0.4" strokeDasharray="3 2"/>
            <circle cx="50" cy="50" r="34" fill="none" stroke="rgba(192,57,43,.08)" strokeWidth="0.2">
              <animate attributeName="stroke-dashoffset" from="0" to="213" dur="8s" repeatCount="indefinite"/>
              <animate attributeName="stroke-dasharray" values="0 213;106 107;213 0;106 107;0 213" dur="4s" repeatCount="indefinite"/>
            </circle>

            {/* Pulsing rings on center */}
            {[14,18,22].map((r, i) => (
              <circle key={i} cx="50" cy="50" r={r} fill="none" stroke="rgba(192,57,43,.2)" strokeWidth="0.3">
                <animate attributeName="r" values={`${r};${r+2};${r}`} dur={`${2+i*.5}s`} repeatCount="indefinite"/>
            ...
            {satellites.map((s, i) => {
              ...
              return (
                <line key={i} x1="50" y1="50" x2={x} y2={y}
                  ...
                  strokeDasharray={isHov ? "none" : "1.5 1"}
            ...
            {satellites.map((s, i) => {
              ...
              return (
                <g key={i} style={{ cursor: "pointer" }}
                  ...
                  {/* Pulsing ring on each satellite */}
                  <circle cx={x} cy={y} r={isHov ? 9 : 7.5} fill="none"
                    stroke="rgba(192,57,43,.35)" strokeWidth="0.4">
                    <animate attributeName="r" values={`${isHov?8:6.5};${isHov?11:9};${isHov?8:6.5}`} dur="2s" repeatCount="indefinite"/>
                  ...
                  />
                  <text x={x} y={y + 0.5} textAnchor="middle" dominantBaseline="middle" fontSize="4" style={{ userSelect: "none" }}>
                    {s.icon}
                  </text>
                  {isHov && (
                    <g>
                      <rect x={x - 18} y={y + 8} width="36" height="9" rx="2" fill="rgba(0,0,0,.85)" stroke="rgba(192,57,43,.5)" strokeWidth="0.3"/>
                      <text x={x} y={y + 10.5} textAnchor="middle" fontSize="2.2" fill="#fff" fontFamily="Raleway,sans-serif" fontWeight="700">{s.label}</text>
                      <text x={x} y={y + 14} textAnchor="middle" fontSize="1.8" fill="rgba(255,255,255,.6)" fontFamily="Raleway,sans-serif">{s.desc}</text>
                    </g>
                  )}
                  {!isHov && (
                    <text x={x} y={y + 8} textAnchor="middle" fontSize="2.2" fill="rgba(255,255,255,.7)" fontFamily="Raleway,sans-serif" fontWeight="600">{s.label}</text>
                  )}
                </g>
              );
            })}

            {/* Center node — Logo image */}
            <circle cx="50" cy="50" r="13" fill="#080808" stroke="#C0392B" strokeWidth="0.7" filter="url(#fg)"/>
            <circle cx="50" cy="50" r="16" fill="none" stroke="rgba(192,57,43,.4)" strokeWidth="0.4">
              <animate attributeName="r" values="13;17;13" dur="2.5s" repeatCount="indefinite"/>
              <animate attributeName="opacity" values="0.7;0;0.7" dur="2.5s" repeatCount="indefinite"/>
            </circle>
            <circle cx="50" cy="50" r="19" fill="none" stroke="rgba(192,57,43,.2)" strokeWidth="0.3">
              <animate attributeName="r" values="15;21;15" dur="3s" repeatCount="indefinite"/>
              <animate attributeName="opacity" values="0.4;0;0.4" dur="3s" repeatCount="indefinite"/>
            </circle>
            <image href={LOGO_URL} x="36" y="36" width="28" height="28" style={{ clipPath: "circle(13px at 14px 14px)" }}/>
          </svg>
        </div>
      </div>
        ...
        {bentoCards.map((card, i) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(192,57,43,.15)", border: "1px solid rgba(192,57,43,.35)", borderRadius: 20, padding: "4px 12px", cursor: "pointer" }}
                onClick={() => {}}>
    ...
    <div style={{ minHeight: "100vh", background: "#080808", color: "#fff", fontFamily: "'Raleway',sans-serif", width: "100%", overflowX: "hidden" }}>
      ...
      <nav className="nav">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <img src={LOGO_URL} alt="Kapital Music" style={{ width: 130, height: 130, objectFit: "contain" }} onError={e=>{e.target.style.display='none';}} />
          <div>
