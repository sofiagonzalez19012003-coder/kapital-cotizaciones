...
const LOGO_URL = CL + "Diseño_sin_título_65_xtjw6m";
const LOGO_NAV = CL + "Diseño_sin_título_66_zdp6ug";
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav { position: sticky; top: 0; z-index: 100; background: rgba(8,8,8,.97); backdrop-filter: blur(16px); border-bottom: 1px solid rgba(89,7,7,.3); padding: 8px 16px; display: flex; align-items: center; justify-content: space-between; width: 100%; min-height: 80px; }
  ...
  .album-rotator { overflow: hidden; width: 100%; margin: 8px 0; }
  .album-track { display: flex; gap: 10px; animation: scroll-albums 45s linear infinite; width: max-content; }
  .album-track-rev { display: flex; gap: 10px; animation: scroll-albums-rev 45s linear infinite; width: max-content; }
  .album-track:hover, .album-track-rev:hover { animation-play-state: paused; }
  @keyframes scroll-albums { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
  @keyframes scroll-albums-rev { 0% { transform: translateX(-50%); } 100% { transform: translateX(0); } }
  ...
  .gallery-img { width: 100%; object-fit: cover; display: block; transition: transform .4s ease, filter .4s ease; cursor: pointer; }
  .gallery-img:active { transform: scale(.97); filter: brightness(1.2); }
  .gallery-wide { grid-column: span 2; }
  @keyframes gallery-reveal { from { opacity:0; transform:scale(.95); } to { opacity:1; transform:scale(1); } }
  .g-reveal { animation: gallery-reveal .55s ease both; }
...
function Preloader({ onDone }) {
  ...
  // stage 0: logo fades in, stage 1: text appears, stage 2: fade out
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", inset: 0, backgroundImage: "linear-gradient(rgba(89,7,7,.08) 1px,transparent 1px),linear-gradient(90deg,rgba(89,7,7,.08) 1px,transparent 1px)", backgroundSize: "40px 40px", opacity: stage >= 1 ? 1 : 0, transition: "opacity 1s" }} />

      {/* Radial glow */}
      <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(89,7,7,.35) 0%, transparent 70%)", top: "50%", left: "50%", transform: "translate(-50%,-50%)" }} />

      {/* Pulsing rings */}
      {[120,160,200].map((s, i) => (
        <div key={i} style={{
          position: "absolute", width: s, height: s, borderRadius: "50%",
          border: `1px solid rgba(89,7,7,${0.4 - i*0.1})`,
          top: "50%", left: "50%", transform: "translate(-50%,-50%)",
          animation: `ring-pulse ${2+i*0.4}s ease-in-out infinite`,
      ...
      <img src={LOGO_URL} alt="Kapital Music" style={{
        width: "65vw", maxWidth: 280, height: "auto", objectFit: "contain", position: "relative",
        animation: "logo-in .8s cubic-bezier(.34,1.56,.64,1) both",
        filter: "drop-shadow(0 0 40px rgba(89,7,7,.9)) drop-shadow(0 0 80px rgba(89,7,7,.4))",
      }} />
      ...
      <div style={{
        ...
        opacity: stage >= 1 ? 1 : 0,
        animation: stage >= 1 ? "text-in .5s ease both" : "none",
        transition: "opacity .5s",
      }}>
        <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: 8, textTransform: "uppercase", color: "#fff" }}>KAPITAL</div>
        <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 10, color: "#C0392B", marginTop: 4 }}>MUSIC</div>
        <div style={{ fontSize: 9, fontWeight: 400, letterSpacing: 3, color: "rgba(255,255,255,.35)", marginTop: 6 }}>SELLO INDEPENDIENTE</div>
      </div>

      {/* Loading bar */}
      <div style={{ marginTop: 40, width: 140, height: 2, background: "rgba(255,255,255,.08)", borderRadius: 2, overflow: "hidden", position: "relative" }}>
        <div style={{ height: "100%", background: "linear-gradient(90deg,#590707,#C0392B)", borderRadius: 2, animation: "bar-fill 2.2s ease both" }} />
      </div>
    </div>
  );
}
...
function LaCasa() {
  ...
  // Primera fila: 3 fotos, segunda: 2, tercera: 3 = 8 fotos visibles
  const PREVIEW_COUNT = 8;
  const visible = expanded ? CASA_IMGS : CASA_IMGS.slice(0, PREVIEW_COUNT);

  // Layout collage: row1=3cols, row2=2cols, row3=3cols
  const getStyle = (i) => {
    if (i === 0) return { gridColumn: "span 1", height: 140 }; // row1
    if (i === 1) return { gridColumn: "span 2", height: 140 }; // row1 wide
    if (i === 2) return { gridColumn: "span 1", height: 140 }; // row1
    if (i === 3) return { gridColumn: "span 2", height: 180 }; // row2 wide
    if (i === 4) return { gridColumn: "span 1", height: 180 }; // row2
    if (i === 5) return { gridColumn: "span 1", height: 140 }; // row3
    if (i === 6) return { gridColumn: "span 1", height: 140 }; // row3
    if (i === 7) return { gridColumn: "span 1", height: 140 }; // row3
    // Expanded rows (same pattern repeating)
  ...
  return (
    <div style={{ background: "#080808" }}>
      {/* HEADER */}
      <div style={{ padding: "36px 18px 20px" }}>
        <div className="tag tag-g" style={{ marginBottom: 12 }}>GALERÍA KAPITAL</div>
        <h2 style={{ fontSize: 32, fontWeight: 900, textTransform: "uppercase", lineHeight: .95, marginBottom: 10, letterSpacing: -1 }}>
          La <span style={{ color: "#C0392B" }}>Casa.</span>
        </h2>
      ...
      <div style={{ position: "relative" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 3, padding: "0 0" }}>
          {visible.map((url, i) => {
            const s = getStyle(i);
            return (
              <div key={i} style={{ gridColumn: s.gridColumn, position: "relative", overflow: "hidden", cursor: "pointer" }}
                onClick={() => setLightbox(i)}>
                <img src={url} alt={"Kapital " + i}
                  style={{ width: "100%", height: s.height, objectFit: "cover", display: "block", filter: "brightness(.82)", transition: "filter .3s, transform .35s" }}
                  onMouseEnter={e => { e.target.style.filter = "brightness(1.05)"; e.target.style.transform = "scale(1.05)"; }}
        ...
        {/* FADE + VER MÁS — solo cuando no está expandido */}
        {!expanded && (
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 160, background: "linear-gradient(to top, #080808 30%, rgba(8,8,8,.7) 70%, transparent 100%)", display: "flex", alignItems: "flex-end", justifyContent: "center", paddingBottom: 20 }}>
            <button
              onClick={() => setExpanded(true)}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, background: "none", border: "none", cursor: "pointer", padding: 0 }}
            >
              <div style={{ background: "rgba(8,8,8,.9)", border: "1px solid rgba(192,57,43,.5)", borderRadius: 30, padding: "10px 24px", display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 800, color: "#fff", letterSpacing: 1.5, textTransform: "uppercase" }}>Ver galería completa</span>
                <span style={{ fontSize: 16, color: "#C0392B" }}>↓</span>
              </div>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)", letterSpacing: 1 }}>{CASA_IMGS.length} fotos</div>
            </button>
          </div>
        )}

        {/* COLAPSAR — cuando está expandido */}
        {expanded && (
          <div style={{ display: "flex", justifyContent: "center", padding: "16px 0 4px" }}>
            <button onClick={() => setExpanded(false)}
              style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 30, padding: "8px 20px", cursor: "pointer", fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,.5)", letterSpacing: 1, textTransform: "uppercase" }}>
              Colapsar ↑
            </button>
          </div>
        )}
      ...
      {/* LIGHTBOX */}
      {lightbox !== null && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(0,0,0,.97)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
          onClick={() => setLightbox(null)}>
          <button onClick={e => { e.stopPropagation(); setLightbox(l => (l - 1 + CASA_IMGS.length) % CASA_IMGS.length); }}
            style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(89,7,7,.8)", border: "none", color: "#fff", fontSize: 24, width: 44, height: 44, borderRadius: "50%", cursor: "pointer" }}>‹</button>
          <img src={CASA_IMGS[lightbox]} alt="Kapital"
            style={{ maxWidth: "100%", maxHeight: "88vh", objectFit: "contain", borderRadius: 6, boxShadow: "0 0 60px rgba(89,7,7,.5)" }}
            onClick={e => e.stopPropagation()} />
          <button onClick={e => { e.stopPropagation(); setLightbox(l => (l + 1) % CASA_IMGS.length); }}
            style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(89,7,7,.8)", border: "none", color: "#fff", fontSize: 24, width: 44, height: 44, borderRadius: "50%", cursor: "pointer" }}>›</button>
          <button onClick={() => setLightbox(null)}
            style={{ position: "absolute", top: 16, right: 16, background: "rgba(89,7,7,.8)", border: "none", color: "#fff", fontSize: 16, width: 36, height: 36, borderRadius: "50%", cursor: "pointer" }}>✕</button>
          <div style={{ position: "absolute", bottom: 20, left: 0, right: 0, textAlign: "center", fontSize: 11, color: "rgba(255,255,255,.35)", letterSpacing: 2 }}>{lightbox + 1} / {CASA_IMGS.length}</div>
        </div>
      )}
    </div>
  );
...
// ─── ECOSYSTEM COMPONENT ──────────────────────────────────────────────────────
function Ecosystem({ onServiceClick }) {
  const [hovered, setHovered] = useState(null);
  const [visibleCards, setVisibleCards] = useState([]);
  const cardRefs = useRef([]);

  const satellites = [
    { label: "Studio", icon: "🎙️", desc: "Grabación profesional de alto nivel", angle: 0, serviceIdx: 0 },
    { label: "Legal", icon: "⚖️", desc: "Protección total de tu música", angle: 60, serviceIdx: 1 },
    { label: "Marketing", icon: "📲", desc: "Estrategia digital y visibilidad", angle: 120, serviceIdx: 3 },
    { label: "Digital", icon: "🌐", desc: "Distribución en todas las plataformas", angle: 180, serviceIdx: 1 },
    { label: "Management", icon: "🤝", desc: "Acompañamiento en tu carrera", angle: 240, serviceIdx: 3 },
    { label: "Sync", icon: "🎬", desc: "Producción y oportunidades", angle: 300, serviceIdx: 4 },
  ];

  const bentoCards = [
    { icon: "🎙️", title: "Estudio Profesional", desc: "Dos estudios totalmente equipados para producciones de la más alta calidad.", color: "#C0392B", serviceIdx: 0 },
    { icon: "⚖️", title: "Blindaje Legal", desc: "Registra, protege y monetiza tu música correctamente.", color: "#8a0c0c", serviceIdx: 1 },
    { icon: "📲", title: "Marketing 360°", desc: "Pauta inteligente, contenido estratégico y crecimiento orgánico real.", color: "#C0392B", serviceIdx: 3 },
    { icon: "🎵", title: "Producción Elite", desc: "TunyD y Money Makers. Del concepto al master en un solo lugar.", color: "#8a0c0c", serviceIdx: 4 },
  ];
  ...
  useEffect(() => {
    const observers = cardRefs.current.map((ref, i) => {
      ...
      const obs = new IntersectionObserver(([entry]) => {
        if (entry.isIntersecting) setTimeout(() => setVisibleCards(v => [...new Set([...v, i])]), i * 120);
      }, { threshold: 0.2 });
      obs.observe(ref);
      return obs;
    });
  ...
  return (
    <div style={{ margin: "20px 0" }}>
      <div style={{ position: "relative", width: "100%", paddingBottom: "110%", maxWidth: 400, margin: "0 auto 28px" }}>
        <div style={{ position: "absolute", inset: 0 }}>
          <svg viewBox="0 0 100 100" style={{ width: "100%", height: "100%", overflow: "visible" }}>
            ...
            <circle cx="50" cy="50" r="36" fill="none" stroke="rgba(192,57,43,.15)" strokeWidth="0.5" strokeDasharray="3 2"/>
            <circle cx="50" cy="50" r="36" fill="none" stroke="rgba(192,57,43,.08)" strokeWidth="0.3">
              <animate attributeName="stroke-dashoffset" from="0" to="226" dur="8s" repeatCount="indefinite"/>
              <animate attributeName="stroke-dasharray" values="0 226;113 113;226 0;113 113;0 226" dur="4s" repeatCount="indefinite"/>
            </circle>
            {[16,21,26].map((r, i) => (
              <circle key={i} cx="50" cy="50" r={r} fill="none" stroke="rgba(192,57,43,.2)" strokeWidth="0.3">
                <animate attributeName="r" values={`${r};${r+2.5};${r}`} dur={`${2+i*.5}s`} repeatCount="indefinite"/>
                <animate attributeName="opacity" values="0.4;0.1;0.4" dur={`${2+i*.5}s`} repeatCount="indefinite"/>
              </circle>
            ))}
            {satellites.map((s, i) => {
              ...
              return (
                <line key={i} x1="50" y1="50" x2={x} y2={y}
                  stroke={isHov ? "rgba(192,57,43,.9)" : "rgba(192,57,43,.25)"}
                  strokeWidth={isHov ? 0.8 : 0.3} strokeDasharray={isHov ? "none" : "1.5 1"}
            ...
            {satellites.map((s, i) => {
              ...
              return (
                <g key={i} style={{ cursor: "pointer" }}
                  ...
                  <circle cx={x} cy={y} r={isHov ? 9 : 7} fill="none" stroke="rgba(192,57,43,.35)" strokeWidth="0.4">
                    <animate attributeName="r" values={`${isHov?8:6};${isHov?12:10};${isHov?8:6}`} dur="2s" repeatCount="indefinite"/>
                    <animate attributeName="opacity" values="0.5;0;0.5" dur="2s" repeatCount="indefinite"/>
                  </circle>
                  <circle cx={x} cy={y} r={isHov ? 7 : 5.5}
                    fill={isHov ? "#C0392B" : "#1a0404"}
                    stroke={isHov ? "#ff6b6b" : "rgba(192,57,43,.5)"}
                    strokeWidth={isHov ? 0.8 : 0.4}
                    filter={isHov ? "url(#fg)" : "none"}
                    style={{ transition: "all .35s" }}
                  />
                  <text x={x} y={y + 0.5} textAnchor="middle" dominantBaseline="middle" fontSize="4" style={{ userSelect: "none" }}>{s.icon}</text>
                  {isHov ? (
                    <g>
                      <rect x={x - 20} y={y + 8} width="40" height="10" rx="2" fill="rgba(0,0,0,.9)" stroke="rgba(192,57,43,.5)" strokeWidth="0.3"/>
                      <text x={x} y={y + 11} textAnchor="middle" fontSize="2.4" fill="#fff" fontFamily="Raleway,sans-serif" fontWeight="700">{s.label}</text>
                      <text x={x} y={y + 15} textAnchor="middle" fontSize="1.9" fill="rgba(255,255,255,.6)" fontFamily="Raleway,sans-serif">{s.desc}</text>
                    </g>
                  ) : (
                    <text x={x} y={y + 9} textAnchor="middle" fontSize="2.4" fill="rgba(255,255,255,.7)" fontFamily="Raleway,sans-serif" fontWeight="600">{s.label}</text>
                  )}
                </g>
              );
            })}
            <circle cx="50" cy="50" r="16" fill="#080808" stroke="#C0392B" strokeWidth="0.8" filter="url(#fg)"/>
            <circle cx="50" cy="50" r="20" fill="none" stroke="rgba(192,57,43,.4)" strokeWidth="0.4">
              <animate attributeName="r" values="16;22;16" dur="2.5s" repeatCount="indefinite"/>
              <animate attributeName="opacity" values="0.7;0;0.7" dur="2.5s" repeatCount="indefinite"/>
            </circle>
            <circle cx="50" cy="50" r="25" fill="none" stroke="rgba(192,57,43,.2)" strokeWidth="0.3">
              <animate attributeName="r" values="18;27;18" dur="3s" repeatCount="indefinite"/>
              <animate attributeName="opacity" values="0.4;0;0.4" dur="3s" repeatCount="indefinite"/>
            </circle>
            <image href={LOGO_NAV} x="31" y="31" width="38" height="38" style={{ clipPath: "circle(16px at 19px 19px)" }}/>
          </svg>
        </div>
      </div>
        ...
        {bentoCards.map((card, i) => {
          ...
          return (
            <div key={i} ref={el => cardRefs.current[i] = el}
              ...
              onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 0 24px rgba(192,57,43,.35)"; e.currentTarget.style.borderColor = "rgba(192,57,43,.5)"; }}
              onMouseLeave={e => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "rgba(255,255,255,.06)"; }}
            >
              <div style={{ width: "100%", height: 48, borderRadius: 8, background: "linear-gradient(135deg, rgba(192,57,43,.12) 0%, rgba(0,0,0,0) 100%)", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", width: 60, height: 60, borderRadius: "50%", background: `radial-gradient(circle, ${card.color}22 0%, transparent 70%)`, top: "50%", left: "50%", transform: "translate(-50%,-50%)" }}/>
                <span style={{ fontSize: 24, position: "relative" }}>{card.icon}</span>
              </div>
              <div style={{ fontWeight: 800, fontSize: 13, color: "#fff", marginBottom: 5, lineHeight: 1.2 }}>{card.title}</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)", lineHeight: 1.55, marginBottom: 12 }}>{card.desc}</div>
              <div
                style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(192,57,43,.15)", border: "1px solid rgba(192,57,43,.35)", borderRadius: 20, padding: "5px 12px", cursor: "pointer", transition: "background .2s" }}
              ...
              >
                <span style={{ fontSize: 10, fontWeight: 700, color: "#C0392B", letterSpacing: .5 }}>Conocer más</span>
...
export default function App() {
    ...
    if (q.type === "multicheck") {
      ...
      if (prev.includes(val)) {
        ...
        if (q.id === "serviciosInteres") {
          // Exclusión mutua: Estudios y Producción
          if (val === "Alquiler de estudios") newVal = newVal.filter(x => x !== "Producción musical completa (incluye mezcla y master)");
          if (val === "Producción musical completa (incluye mezcla y master)") {
            newVal = newVal.filter(x => x !== "Alquiler de estudios");
            // Producción incluye mezcla — quitar Mezcla & Mastering
  ...
  const buildAndSubmit = async (extrasData) => {
    ...
    // Si eligió Producción, quitar Mezcla de la cotización (viene incluida)
    ...
    // Generar análisis profundo localmente
    ...
    // const sello = answers.selloInversionista || "";
    ...
    const tabla = [];
    const estadoIG = segIG.includes("Menos de 1,000") || segIG.includes("No tengo") ? "Audiencia muy pequeña" : segIG.includes("1,000 – 5,000") ? "Audiencia emergente" : segIG.includes("5,000") || segIG.includes("20,000") ? "En crecimiento" : "Audiencia establecida";
    tabla.push({ area: "Presencia digital", estado: estadoIG, potencial: "Con estrategia de contenido + pauta Meta podemos escalar tu alcance mes a mes" });
    const estadoProd = lanz.includes("Ninguno") ? "Sin material lanzado aún" : lanz.includes("1 – 3") ? "Primeros lanzamientos" : lanz.includes("4 – 10") ? "Catálogo en construcción" : "Catálogo establecido";
    tabla.push({ area: "Producción musical", estado: estadoProd, potencial: "Sonido profesional con TunyD o Money Makers — mezcla y master incluidos" });
    tabla.push({ area: "Protección legal", estado: registrado === "Sí" ? "Registrado ✓" : "Sin registro legal ⚠", potencial: registrado === "Sí" ? "Optimizar contratos y acuerdos comerciales" : "Registro DNDA + SAYCO/ACINPRO/ASCAP desde la primera sesión" });
    tabla.push({ area: "Distribución", estado: distribuidora === "No tengo" ? "Sin distribuidora" : `Con ${distribuidora}`, potencial: distribuidora === "No tengo" ? "Asesoría para elegir la mejor plataforma según tu perfil" : "Estrategia de lanzamiento para maximizar streams y royalties" });
    tabla.push({ area: "Equipo / Respaldo", estado: management.includes("solo") ? "Trabajando solo" : management, potencial: "Kapital como equipo completo: producción, estrategia y legal en un solo lugar" });
    ...
    if (contenido.includes("No publico") || contenido.includes("Esporádicamente")) frena.push(`La inconsistencia en redes es uno de los mayores frenos para crecer hoy. Sin contenido constante el algoritmo no trabaja para ti y nuevos fans no te descubren.`);
    if (lanz.includes("Ninguno")) frena.push(`Sin lanzamientos oficiales no hay historial digital que las plataformas, prensa o booking agencies puedan verificar. Tu carrera necesita ese primer ancla.`);
    if (management.includes("solo") && asesoriaPrevia !== "Sí") frena.push(`Estás gestionando todo solo y sin asesoría previa — lo que significa que probablemente estés dejando dinero sobre la mesa en negociaciones y contratos.`);
    ...
    if (generos) fortalezas.push(`Tu identidad en ${generos} te da un nicho claro — lo que facilita la estrategia de contenido y posicionamiento.`);
    if (contenido.includes("Casi diario") || contenido.includes("equipo")) fortalezas.push(`Tu disciplina con el contenido digital es una ventaja competitiva real en un mercado donde la mayoría no es constante.`);
    if (fortalezas.length === 0) fortalezas.push(`Tu decisión de buscar profesionalización en este momento es exactamente lo que diferencia a los artistas que crecen de los que se quedan estancados.`);
    ...
    const pasos = [];
    if (registrado !== "Sí") pasos.push(`Registra tu música ante la DNDA y afíliate a SAYCO/ACINPRO. Nuestro servicio de Asesoría Legal lo resuelve en una sola sesión — después de esto tu música empieza a generar regalías.`);
    if (distribuidora === "No tengo") pasos.push(`Configura tu distribuidora digital. En la asesoría te orientamos según tu volumen de lanzamientos, géneros y países objetivo para que elijas la que más te conviene.`);
    if (serviciosFiltrados.includes("Producción musical completa (incluye mezcla y master)")) pasos.push(`Reserva tu sesión de producción — el proceso incluye concepto, grabación, mezcla y master. En la reunión definimos el número de sesiones exactas para tu proyecto.`);
    if (serviciosFiltrados.includes("Marketing 30")) pasos.push(`Activar Marketing 30: análisis de tu presencia digital actual, definición de contenido por semana, estrategia de pauta en Meta y plan de crecimiento orgánico con métricas desde el primer mes.`);
    if (serviciosFiltrados.includes("Alquiler de estudios")) pasos.push(`Agenda tu sesión de estudio — te ayudamos a elegir entre Estudio A y B según tu proyecto, y coordinamos el ingeniero que mejor se adapte a tu género.`);
    pasos.push(`Agenda una reunión con el equipo Kapital — es gratuita, sin compromiso, y en ella ajustamos todos los detalles de esta propuesta para que arranquemos lo antes posible.`);
    ...
    <div style={{ minHeight: "100vh", background: "#080808", color: "#fff", fontFamily: "'Raleway',sans-serif", width: "100%", overflowX: "hidden" }}>
      ...
      <nav className="nav">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <img src={LOGO_NAV} alt="Kapital Music" style={{ width: 160, height: 160, objectFit: "contain", display: "block", flexShrink: 0 }} />
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: 3, textTransform: "uppercase", color: "#fff" }}>Kapital Music</div>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,.35)", letterSpacing: 2, textTransform: "uppercase" }}>Sello Independiente</div>
          </div>
        </div>
      ...
      {phase === "landing" && (
        <div style={{ width: "100%" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <div className="fu" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 28 }}>
                {[
                  ...
                  <div key={i} style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(89,7,7,.3)", borderRadius: 14, padding: "16px 8px", textAlign: "center", boxShadow: "0 0 20px rgba(89,7,7,.12)" }}>
                    <div style={{ fontWeight: 900, fontSize: s.target === null ? 28 : 28, color: "#590707", lineHeight: 1 }}>
                      {s.target !== null
                        ? <CountUp target={s.target} prefix={s.prefix} suffix="" />
                        : <span style={{ fontSize: 26 }}>{s.icon}</span>}
                    </div>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,.45)", letterSpacing: 1, marginTop: 6, textTransform: "uppercase", lineHeight: 1.4 }}>{s.l}</div>
                  </div>
                ))}
              </div>
          ...
          {/* PORTADAS ROTATORIAS — DOS HILERAS */}
          <div style={{ padding: `28px 0`, overflow: "hidden" }}>
            ...
            <div style={{ height: 10 }} />
            <div className="album-rotator">
              <div className="album-track-rev">
                {[...PORTADAS_ROW2, ...PORTADAS_ROW2].map((url, i) => (
                  <img key={i} src={url} alt={"portada2 " + i} className="album-cover" onError={e=>{e.target.style.display="none";}} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{ background: "rgba(89,7,7,.08)", borderTop: "1px solid rgba(89,7,7,.2)", borderBottom: "1px solid rgba(89,7,7,.2)", padding: `28px ${P}` }}>
            ...
            <p style={{ fontSize: 13, color: "rgba(255,255,255,.5)", lineHeight: 1.65, marginBottom: 0 }}>
              Kapital conecta cada pieza de tu carrera artística en un solo lugar — producción, estrategia, protección legal y visibilidad, todo trabajando en sincronía para que tú solo te enfoques en crear.
            </p>
            ...
            <div style={{ marginTop: 20 }}>
              <button className="btn-ghost" onClick={() => setPhase("services")} style={{ width: "100%" }}>Ver todos los servicios →</button>
            </div>
          </div>
      ...
      {phase === "form" && q && (
        <div style={{ padding: `20px ${P}`, maxWidth: 500, margin: "0 auto" }}>
          ...
          <div key={currentQ} className="fu">
            ...
            {q.type === "multicheck" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {(q.filterOptions ? q.filterOptions(answers) : q.options).map(opt => { const sel = (answers[q.id] || []).includes(opt); return (
      ...
      {phase === "proposal" && proposal && cotizacion && (
        <div style={{ padding: `16px ${P}`, maxWidth: 500, margin: "0 auto" }}>
          ...
          <div className="card fu" style={{ marginBottom: 14 }}>
            ...
            <p style={{ color: "rgba(255,255,255,.72)", fontSize: 14, lineHeight: 1.8, marginBottom: 20, fontStyle: "italic" }}>{proposal.diagnostico?.donde_esta}</p>

            {/* TABLA DIAGNÓSTICO — rediseñada */}
            {proposal.diagnostico?.tabla?.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 2, color: "rgba(255,255,255,.35)", textTransform: "uppercase", marginBottom: 12 }}>📊 Tu panorama actual</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {proposal.diagnostico.tabla.map((row, i) => (
                    <div key={i} style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
                      <div style={{ fontSize: 11, fontWeight: 800, color: "#fff", letterSpacing: .5 }}>{row.area}</div>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                        <span style={{ fontSize: 11, color: "rgba(255,120,120,.9)", background: "rgba(89,7,7,.2)", border: "1px solid rgba(89,7,7,.35)", borderRadius: 4, padding: "2px 8px" }}>{row.estado}</span>
                        <span style={{ fontSize: 10, color: "rgba(255,255,255,.3)" }}>→</span>
                        <span style={{ fontSize: 11, color: "#6be8a0", lineHeight: 1.4 }}>{row.potencial}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {proposal.diagnostico?.que_lo_frena?.length > 0 && (
              <div style={{ background: "rgba(89,7,7,.12)", border: "1px solid rgba(89,7,7,.25)", borderRadius: 10, padding: "14px", marginBottom: 16 }}>
                <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.5, color: "#ff8080", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>🚧 Lo que te está frenando hoy</div>
                {proposal.diagnostico.que_lo_frena.map((f, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, marginBottom: i < proposal.diagnostico.que_lo_frena.length - 1 ? 8 : 0, color: "rgba(255,255,255,.75)", fontSize: 13, lineHeight: 1.55 }}>
                    <span style={{ color: "#ff8080", flexShrink: 0, marginTop: 1 }}>·</span>{f}
                  </div>
                ))}
              </div>
            )}
            ...
            {proposal.diagnostico?.fortalezas?.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.5, color: "#6be8a0", textTransform: "uppercase", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>✦ Lo que ya tienes a tu favor</div>
                {proposal.diagnostico.fortalezas.map((f, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, alignItems: "flex-start" }}>
                    <div style={{ width: 20, height: 20, borderRadius: "50%", background: "rgba(26,143,60,.2)", border: "1px solid rgba(26,143,60,.4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#6be8a0", flexShrink: 0, marginTop: 1 }}>✓</div>
                    <span style={{ fontSize: 13, color: "rgba(255,255,255,.72)", lineHeight: 1.55 }}>{f}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {proposal.diagnostico?.pasos_a_seguir?.length > 0 && (
              <div style={{ background: "rgba(89,7,7,.08)", border: "1px solid rgba(89,7,7,.2)", borderRadius: 10, padding: "14px" }}>
                <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.5, color: "#590707", textTransform: "uppercase", marginBottom: 12 }}>→ Por dónde arrancar</div>
                {proposal.diagnostico.pasos_a_seguir.slice(0, 4).map((p, i) => (
                  <div key={i} style={{ display: "flex", gap: 12, marginBottom: i < 3 ? 10 : 0, alignItems: "flex-start" }}>
                    <div style={{ width: 22, height: 22, borderRadius: "50%", background: "#590707", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 900, color: "#fff", flexShrink: 0, marginTop: 1 }}>{i + 1}</div>
                    <span style={{ fontSize: 12, color: "rgba(255,255,255,.72)", lineHeight: 1.55 }}>{p}</span>
                  </div>
                ))}
              </div>
            )}
