...
function LaCasa() {
  ...
  useEffect(() => {
    const obs = refs.current.map((ref, i) => {
      ...
      const o = new IntersectionObserver(([e]) => {
        if (e.isIntersecting) setTimeout(() => setVisible(v => [...new Set([...v, i])]), i * 60);
      }, { threshold: 0.1 });
      o.observe(ref);
      return o;
    });
  ...
  // Layout: wide, pair, pair, wide, pair, pair, wide, pair, single
  const layout = [
    { wide: true },   // 0 — full width
    { wide: false },  // 1
    { wide: false },  // 2
    { wide: false },  // 3
    { wide: false },  // 4
    { wide: true },   // 5 — full width
    { wide: false },  // 6
    { wide: false },  // 7
    { wide: false },  // 8
    { wide: false },  // 9
    { wide: true },   // 10 — full width
    { wide: false },  // 11
    { wide: false },  // 12
    { wide: false },  // 13
    { wide: false },  // 14
    { wide: true },   // 15 — full width
    { wide: false },  // 16
  ];
  ...
  return (
    <div style={{ padding: "0 0 0 0", background: "#080808" }}>
      {/* HEADER */}
      <div style={{ padding: "36px 18px 24px" }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,.45)", lineHeight: 1.65 }}>
          El lugar donde la música toma forma. Nuestros espacios, nuestro ambiente — el ambiente que tu carrera merece.
        </p>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                opacity: isVis ? 1 : 0,
                transform: isVis ? "scale(1)" : "scale(.95)",
                transition: `opacity .55s ease ${i * 0.05}s, transform .55s ease ${i * 0.05}s`,
      ...
      <div style={{ padding: "20px 18px 0", textAlign: "center" }}>
        <p style={{ fontSize: 12, color: "rgba(255,255,255,.3)", letterSpacing: 1 }}>TOCA CUALQUIER FOTO PARA VER EN DETALLE</p>
      </div>

      {/* LIGHTBOX */}
      {lightbox !== null && (
        <div
          style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(0,0,0,.96)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
          onClick={() => setLightbox(null)}
        >
          {/* Prev */}
          <button onClick={e => { e.stopPropagation(); setLightbox(l => (l - 1 + CASA_IMGS.length) % CASA_IMGS.length); }}
            style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(89,7,7,.7)", border: "none", color: "#fff", fontSize: 22, width: 40, height: 40, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
          <img src={CASA_IMGS[lightbox]} alt="Kapital" style={{ maxWidth: "100%", maxHeight: "88vh", objectFit: "contain", borderRadius: 6, boxShadow: "0 0 60px rgba(89,7,7,.4)" }} onClick={e => e.stopPropagation()} />
          {/* Next */}
          <button onClick={e => { e.stopPropagation(); setLightbox(l => (l + 1) % CASA_IMGS.length); }}
            style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(89,7,7,.7)", border: "none", color: "#fff", fontSize: 22, width: 40, height: 40, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
          {/* Close */}
          <button onClick={() => setLightbox(null)}
            style={{ position: "absolute", top: 16, right: 16, background: "rgba(89,7,7,.8)", border: "none", color: "#fff", fontSize: 18, width: 36, height: 36, borderRadius: "50%", cursor: "pointer" }}>✕</button>
          {/* Counter */}
          <div style={{ position: "absolute", bottom: 20, left: 0, right: 0, textAlign: "center", fontSize: 11, color: "rgba(255,255,255,.4)", letterSpacing: 2 }}>{lightbox + 1} / {CASA_IMGS.length}</div>
        </div>
      )}
    </div>
  );
