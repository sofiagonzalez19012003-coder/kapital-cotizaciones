...
function calcularPropuesta(serviciosElegidos, extras, esInternacional) {
  ...
  if (tiene("Alquiler de estudios")) {
    ...
    const totalEst = (esPromo ? p.horaPromo : p.hora) * horas * sesiones;
    totalBase += p.hora * horas * sesiones; totalPromo += totalEst;
    items.push({ nombre: `Estudio ${est}${esInternacional ? " (virtual)" : ""}`, detalle: `${sesiones} sesión${sesiones > 1 ? "es" : ""} × ${horas}h`, precioUnit: fmt(esPromo ? p.horaPromo : p.hora) + "/hora", total: fmt(totalEst), promo: esPromo, promoRazon: esInternacional ? "sesión virtual" : sesiones > 3 ? `${sesiones} sesiones` : "combinado con otro servicio" });
  }
  ...
  if (tiene("Producción musical completa (incluye mezcla y master)")) {
    ...
    const totalProd = (esPromo ? p.sesionPromo : p.sesion) * sesiones;
    totalBase += p.sesion * sesiones; totalPromo += totalProd;
    items.push({ nombre: prod === "tunyD" ? "Producción con TunyD (incl. mezcla y master)" : "Producción con Money Makers (incl. mezcla y master)", detalle: `${sesiones} sesión${sesiones > 1 ? "es" : ""}${esInternacional ? " (virtual)" : ""}`, precioUnit: fmt(esPromo ? p.sesionPromo : p.sesion) + "/sesión", total: fmt(totalProd), promo: esPromo, promoRazon: esInternacional ? "sesión virtual" : `${sesiones} sesiones` });
  }
  ...
  if (tiene("Asesoría legal")) {
    ...
    items.push({ nombre: "Asesoría Legal", detalle: "Sesión única", precioUnit: fmt(PRECIOS.asesoria.base), total: fmt(precio), promo: esPromo, promoRazon: esPromo ? "combinado con Marketing 30" : null });
  }
  ...
  if (tiene("Marketing 30")) {
    ...
    items.push({ nombre: "Marketing 30", detalle: "Plan mensual completo", precioUnit: fmt(PRECIOS.marketing.base) + "/mes", total: fmt(precio), promo: esPromo, promoRazon: esPromo ? "combinado con otro servicio" : null });
  }
  ...
  if (tiene("Mezcla & Mastering (ya tengo grabado)")) {
    ...
    const totalMM = (esPromo ? PRECIOS.mezclamaster.promoMas3 : PRECIOS.mezclamaster.base) * canciones;
    totalBase += PRECIOS.mezclamaster.base * canciones; totalPromo += totalMM;
    items.push({ nombre: "Mezcla & Mastering", detalle: `${canciones} canción${canciones > 1 ? "es" : ""}`, precioUnit: fmt(esPromo ? PRECIOS.mezclamaster.promoMas3 : PRECIOS.mezclamaster.base) + "/canción", total: fmt(totalMM), promo: esPromo, promoRazon: esPromo ? (esInternacional ? "cliente internacional" : `${canciones} canciones`) : null });
  }

  return { items, totalBase, totalPromo, ahorro: totalBase - totalPromo };
}
...
// ─── SYSTEM PROMPT ────────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `Eres el equipo de Kapital Music escribiéndole directamente a un artista. Tono de tú a tú: cercano, directo, honesto. Como un manager con experiencia que te dice la verdad con respeto.

Kapital Music: 15 años en la industria, +30 artistas activos, +100 canciones producidas, visibilidad internacional. Servicios: estudio, asesoría legal, mezcla & mastering, marketing 30, producción musical.

Tu tarea es hacer un análisis PROFUNDO y HONESTO del artista. Devuelve SOLO este JSON (sin markdown):
{
  "bienvenida": "2 frases directas y cálidas con el nombre artístico. Que sienta que lo conoces.",
  "diagnostico": {
    "nivel": "COMENZANDO | CRECIENDO | ESTABLECIDO",
    "donde_esta": "2-3 frases honestas sobre su momento actual. Sin endulzar, sin herir. Basado en sus datos reales.",
    "que_lo_frena": ["2-3 obstáculos concretos y específicos que lo están limitando ahora mismo, basados en sus respuestas"],
    "fortalezas": ["2-3 cosas reales que tiene a su favor"],
    "pasos_a_seguir": ["3-4 acciones concretas y ordenadas que debe tomar, empezando por lo más urgente. Sé específico, no genérico."]
  },
  "mensajeServicios": "2 frases naturales conectando su situación con los servicios que seleccionó.",
  "mensajeFinal": "2 frases de cierre que lo motiven a tomar acción. Directo, sin clichés."
}
Si no está registrado → pasos incluye registro como paso 1. Sin distribuidora → menciónalo. Adapta tono al género.`;
...
const SERVICES = [
  { icon: "🎙️", title: "Alquiler de Estudios", tag: "GRABACIÓN", desc: "Estudio A (premium) y Estudio B disponibles, ambos con ingenieros certificados. Por hora o en paquetes. Los precios los conoces después de tu consulta personalizada." },
  { icon: "⚖️", title: "Asesoría Legal", tag: "PROTECCIÓN", desc: "Contratos, registro SAYCO/ACINPRO, distribuidoras y todo lo que necesitas para que tu carrera sea un negocio real." },
  { icon: "🎚️", title: "Mezcla & Mastering", tag: "PRODUCCIÓN", desc: "¿Ya tienes tus temas grabados? Nuestros ingenieros los llevan al nivel internacional que merecen. Entrega en 72h." },
  { icon: "📲", title: "Marketing 30", tag: "CRECIMIENTO", desc: "Análisis de contenido, crecimiento orgánico, pauta en Meta, parrilla mensual y asesoramiento de marca. Un mes completo de activación." },
  { icon: "🎵", title: "Producción Musical", tag: "CREACIÓN", desc: "TunyD y Money Makers. Desde el concepto hasta el master — beat, arreglos, grabación, mezcla y mastering incluidos." },
];
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nivel-badge { display: inline-block; padding: 5px 12px; border-radius: 4px; font-size: 9px; font-weight: 800; letter-spacing: 2px; }
...
export default function App() {
  ...
  const buildAndSubmit = async (extrasData) => {
    ...
    setSavedOk(true);
    const summary = visibleQuestions.map(q => { const a = answers[q.id]; if (!a || (Array.isArray(a) && !a.length)) return null; return `${q.label}: ${Array.isArray(a) ? a.join(", ") : a}`; }).filter(Boolean).join("\n");
    const extrasSummary = Object.entries(extrasData).map(([k, v]) => `${k}: ${v}`).join(", ");
    const intlNote = internacional ? `\nNOTA: Artista internacional (${answers.ciudad}) — aplican precios virtuales equivalentes a +3 sesiones.` : "";
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1500, system: SYSTEM_PROMPT, messages: [{ role: "user", content: `Analiza este perfil y entrega el JSON:\n\n${summary}\n\nExtras: ${extrasSummary}${intlNote}` }] }) });
      ...
      {phase === "landing" && (
        <div style={{ width: "100%" }}>
          ...
          <div style={{ background: "rgba(89,7,7,.08)", borderTop: "1px solid rgba(89,7,7,.2)", borderBottom: "1px solid rgba(89,7,7,.2)", padding: `28px ${P}`, marginBottom: 0 }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 900, marginBottom: 18, textTransform: "uppercase", lineHeight: 1.1 }}>Lo que nos hace<br />diferentes</h2>
            {[
              { ico: "🏠", title: "Todo bajo un mismo techo", desc: "No tienes que ir a 5 lugares distintos. Studio, legal, marketing y producción en un solo equipo." },
              { ico: "🤝", title: "Trato directo, sin intermediarios", desc: "Trabajas con el equipo real. Sin capas, sin burocracia, sin esperas innecesarias." },
              { ico: "📈", title: "Resultados comprobados", desc: "+50 artistas lo han vivido. Más de 250 canciones producidas con visibilidad real." },
              { ico: "🌎", title: "Alcance internacional", desc: "Trabajamos con artistas de toda Latinoamérica. Sesiones virtuales disponibles." },
              { ico: "⚡", title: "Entrega rápida", desc: "Mezcla y master en 72h. Estrategia de contenidos lista en la primera semana." },
            ].map((item, i) => (
              <div key={i} style={{ display: "flex", gap: 14, marginBottom: 16, alignItems: "flex-start" }}>
                <div style={{ width: 36, height: 36, background: "rgba(89,7,7,.3)", border: "1px solid rgba(89,7,7,.5)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{item.ico}</div>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 4 }}>{item.title}</div>
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,.52)", lineHeight: 1.55 }}>{item.desc}</div>
                </div>
              </div>
            ))}
          </div>
                ...
                {activeSrv === i && (
                  <div className="srv-detail fu">
                    <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                      ...
                      <div className="tag tag-g" style={{ fontSize: 9 }}>{s.tag}</div>
                    </div>
                    <p style={{ color: "rgba(255,255,255,.65)", fontSize: 14, lineHeight: 1.75, marginBottom: 16 }}>{s.desc}</p>
        ...
        <div style={{ padding: `16px ${P}`, maxWidth: 500, margin: "0 auto" }}>
          ...
          <div className="card fu" style={{ marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
              <div>
                <div className="tag tag-g" style={{ marginBottom: 6 }}>ANÁLISIS DE TU CARRERA</div>
                <h2 style={{ fontSize: 16, fontWeight: 800 }}>En qué punto estás y qué te frena</h2>
              </div>
            ...
            <p style={{ color: "rgba(255,255,255,.65)", fontSize: 13, lineHeight: 1.7, marginBottom: 14 }}>{proposal.diagnostico?.donde_esta}</p>
            ...
            {proposal.diagnostico?.fortalezas?.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                ...
                {proposal.diagnostico.fortalezas.map((f, i) => (
                  <div key={i} className="cr"><div className="ci" style={{ background: "rgba(26,143,60,.2)", borderColor: "rgba(26,143,60,.4)", color: "#6be8a0" }}>✓</div><span style={{ fontSize: 13, color: "rgba(255,255,255,.72)", lineHeight: 1.5 }}>{f}</span></div>
                ))}
              </div>
            )}
              ...
              <div>
                ...
                {proposal.diagnostico.pasos_a_seguir.map((p, i) => (
                  <div key={i} style={{ display: "flex", gap: 12, marginBottom: 12, alignItems: "flex-start" }}>
                    ...
                    <span style={{ fontSize: 13, color: "rgba(255,255,255,.78)", lineHeight: 1.55, paddingTop: 4 }}>{p}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* COTIZACIÓN — PAQUETE COMPLETO */}
          {cotizacion.items.length > 1 && (
            <div className="pkg-box fu" style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                <div>
                  <div className="tag" style={{ marginBottom: 6 }}>PAQUETE COMPLETO</div>
                  <h3 style={{ fontSize: 15, fontWeight: 800 }}>Todos tus servicios juntos</h3>
                </div>
                {cotizacion.ahorro > 0 && <div className="tag tag-y">Ahorras {fmt(cotizacion.ahorro)}</div>}
              </div>
              {cotizacion.items.map((item, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 8, marginBottom: 8, borderBottom: i < cotizacion.items.length - 1 ? "1px solid rgba(255,255,255,.06)" : "none" }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>{item.nombre}</div>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,.38)" }}>{item.detalle}</div>
                    {item.promo && item.promoRazon && <div className="promo-pill">🏷 {item.promoRazon}</div>}
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 900, color: item.promo ? "#f0d060" : "#fff" }}>{item.total}</div>
                  </div>
                </div>
              ))}
              <div className="divl" />
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                <div>
                  <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: 2, color: "rgba(255,255,255,.38)", marginBottom: 2 }}>TOTAL PAQUETE</div>
                  {cotizacion.ahorro > 0 && <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)", textDecoration: "line-through" }}>{fmtFull(cotizacion.totalBase)} COP</div>}
                </div>
                <div style={{ fontSize: 22, fontWeight: 900 }}>{fmtFull(cotizacion.totalPromo)} COP</div>
              </div>
            </div>
          )}

          {/* COTIZACIÓN — SERVICIOS INDIVIDUALES */}
          <div className="ind-box fu" style={{ marginBottom: 14 }}>
            <div className="tag tag-g" style={{ marginBottom: 10 }}>PRECIOS INDIVIDUALES</div>
            <h3 style={{ fontSize: 15, fontWeight: 800, marginBottom: 6 }}>También puedes comenzar con un solo servicio</h3>
            <p style={{ color: "rgba(255,255,255,.4)", fontSize: 12, lineHeight: 1.6, marginBottom: 14 }}>{proposal.mensajeServicios}</p>
            {cotizacion.items.map((item, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,.03)", border: `1px solid ${item.promo ? "rgba(89,7,7,.3)" : "rgba(255,255,255,.07)"}`, borderRadius: 10, padding: "13px", marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 2 }}>{item.nombre}</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,.38)", marginBottom: 2 }}>{item.detalle}</div>
                    {item.precioUnit && <div style={{ fontSize: 10, color: "rgba(255,255,255,.3)" }}>Precio unitario: {item.precioUnit}</div>}
                    {item.promo && item.promoRazon && <div className="promo-pill">🏷 {item.promoRazon}</div>}
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 900, color: item.promo ? "#f0d060" : "#fff" }}>{item.total}</div>
            ...
            {esIntl && (
              <div className="int-banner" style={{ marginTop: 10 }}>
                <span>🌎</span>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,.6)" }}>Precios con tarifa internacional aplicada. Para pagos en USD contáctanos directamente.</div>
              </div>
            )}
          </div>
          ...
          <div style={{ background: "linear-gradient(135deg,rgba(26,143,60,.16),rgba(15,77,40,.12))", border: "1px solid rgba(26,143,60,.35)", borderRadius: 14, padding: "20px", marginBottom: 18 }}>
            ...
            {["Sin costo — sin compromiso", "Respondemos en menos de 24 horas", "Virtual o presencial"].map((b, i) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 13, color: "rgba(255,255,255,.62)", marginBottom: 6 }}><span style={{ color: "#6be8a0" }}>✓</span>{b}</div>
            ))}
