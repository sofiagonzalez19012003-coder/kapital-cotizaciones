  ...
  return (
    <div style={{
      ...
      <img src={LOGO_URL} alt="Kapital Music" style={{
        width: 140, height: 140, objectFit: "contain", position: "relative",
        animation: "logo-in .7s cubic-bezier(.34,1.56,.64,1) both",
        filter: "drop-shadow(0 0 30px rgba(89,7,7,.8))",
      }} />

      {/* Text */}
      <div style={{
        marginTop: 20, textAlign: "center", position: "relative",
        opacity: stage >= 1 ? 1 : 0,
        animation: stage >= 1 ? "text-in .5s ease both" : "none",
      }}>
        <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: 6, textTransform: "uppercase", color: "#fff" }}>KAPITAL</div>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 8, color: "#590707", marginTop: 2 }}>MUSIC</div>
      </div>

      {/* Loading bar */}
      <div style={{ marginTop: 32, width: 120, height: 2, background: "rgba(255,255,255,.08)", borderRadius: 2, overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ position: "relative", width: "100%", paddingBottom: "110%", maxWidth: 400, margin: "0 auto 28px" }}>
        <div style={{ position: "absolute", inset: 0 }}>
          <svg viewBox="0 0 100 100" style={{ width: "100%", height: "100%", overflow: "visible" }}>
            ...
            <circle cx="50" cy="50" r="14" fill="#080808" stroke="#C0392B" strokeWidth="0.8" filter="url(#fg)"/>
            <circle cx="50" cy="50" r="17" fill="none" stroke="rgba(192,57,43,.4)" strokeWidth="0.4">
              <animate attributeName="r" values="14;19;14" dur="2.5s" repeatCount="indefinite"/>
              <animate attributeName="opacity" values="0.7;0;0.7" dur="2.5s" repeatCount="indefinite"/>
            </circle>
            <circle cx="50" cy="50" r="21" fill="none" stroke="rgba(192,57,43,.2)" strokeWidth="0.3">
              <animate attributeName="r" values="16;23;16" dur="3s" repeatCount="indefinite"/>
              <animate attributeName="opacity" values="0.4;0;0.4" dur="3s" repeatCount="indefinite"/>
            </circle>
            <image href={LOGO_URL} x="35" y="35" width="30" height="30" style={{ clipPath: "circle(14px at 15px 15px)" }}/>
          </svg>
        </div>
      </div>
...
export default function App() {
  ...
  const buildAndSubmit = async (extrasData) => {
    ...
    let donde = `${nombre}, llevas ${anos} activo en la música`;
    if (lanz.includes("Ninguno")) donde += ` y aún no has lanzado material oficial — lo que significa que estás en la etapa más crucial: definir con quién y cómo dar ese primer paso.`;
    else donde += ` y ya tienes ${lanz} lanzamientos oficiales, lo que demuestra que has dado pasos reales en tu carrera.`;
    donde += ` Tu presencia en Instagram es de ${segIG} seguidores`;
    if (segTT && !segTT.includes("No tengo")) donde += ` y en TikTok cuentas con ${segTT}`;
    donde += `.`;
    if (contenido.includes("No publico")) donde += ` Actualmente no estás publicando contenido de forma activa, lo que hace casi invisible tu trabajo ante nuevas audiencias.`;
    else if (contenido.includes("Esporádicamente")) donde += ` Tu publicación de contenido es esporádica — el algoritmo necesita consistencia para empujarte.`;
    else if (contenido.includes("Casi diario") || contenido.includes("equipo")) donde += ` Tu actividad digital es sólida, lo cual es una ventaja real.`;
    if (descripcion) donde += ` Sobre tu proyecto: "${descripcion}".`;
    if (sello && !sello.includes("independiente")) donde += ` También vemos que ${sello.toLowerCase()}, lo cual puede ser una fortaleza si se gestiona bien.`;
    ...
    setProposal({
      bienvenida: `${nombre}, analizamos tu perfil completo y armamos esta propuesta específicamente para ti — acá lo que vemos y cómo Kapital puede transformar tu carrera.`,
      ...
      mensajeServicios: `Estos son los servicios que seleccionaste. Cada uno está justificado con lo que vemos en tu perfil.`,
      mensajeFinal: `${nombre}, el talento ya lo tienes. Lo que necesitas es el equipo y la estructura correcta — y eso es exactamente lo que Kapital te ofrece.`
    });
    setPhase("proposal");
  };
